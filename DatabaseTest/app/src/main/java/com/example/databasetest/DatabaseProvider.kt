...
class DatabaseProvider : ContentProvider() {
    ...
    private val bookItem = 0
    private val categoryDir = 0
    private val categoryItem = 0
    ...
    override fun insert(uri: Uri, values: ContentValues?) = dbHelper?.let {
        ...
        val uriReturn = when (uriMatcher.match(uri)) {
            ...
            categoryDir, categoryItem -> {
                val newCategotyId = db.insert("Category", null, values)
                Uri.parse("content://$authority/category/$newCategotyId")
            }
    ...
    override fun query(
        ...
        val cursor = when (uriMatcher.match(uri)) {
            ...
            categoryItem -> {
                val categoryId = uri.pathSegments[1]
                db.query(
                    "Category", projection, "id = ?", arrayOf(categoryId), null, null, sortOrder
                )
            }
